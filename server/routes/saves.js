...
router.post('/', (req, res) => {
  try {
    const { world_id, name, game_state } = req.body;
    ...
    const stmt = db.prepare(`
      INSERT INTO saves (user_id, world_id, name, game_state)
      VALUES (?, ?, ?, ?)
    `);
...
router.put('/:id', (req, res) => {
  try {
    ...
    `);
    const result = stmt.run(JSON.stringify(game_state), req.params.id, req.user.id);
